...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        $error = 'Il titolo dell\'articolo è obbligatorio.';
    } elseif (empty($description)) {
        $error = 'La descrizione è obbligatoria.';
    } elseif (!is_numeric($price) || (float)$price <= 0) {
        $error = 'Inserisci un prezzo valido maggiore di 0.';
    } else {
        ...
        if (!empty($_FILES['photo']['name']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($file['size'] > 5 * 1024 * 1024) {
                ...
                $filename  = 'shop_' . $currentUserId . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
                $uploadDir = rtrim($_SERVER['DOCUMENT_ROOT'], '/\\') . '/tastegram/img/uploads/shops/';
                ...
                if (move_uploaded_file($file['tmp_name'], $uploadDir . $filename)) {
                    ...
                    $error = 'Errore durante il caricamento. Controlla i permessi di img/uploads/shops/';
                }
            }
        ...
        if (empty($error)) {
            $stmt = $sql->prepare("
                INSERT INTO shop_items (user_id, title, description, price, image_path)
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([$currentUserId, $title, $description, (float)$price, $imagePath]);

            $newId = (int) $sql->lastInsertId();
            header('Location: shop_item.php?id=' . $newId);
            exit;
        }
    }
}
...
<head>
    ...
    <title>Vendi un articolo — Tastegram</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .price-wrap .currency-symbol {
            ...
            font-size: 16px; color: var(--br); font-weight: 700; pointer-events: none;
        }
        .price-wrap input { padding-left: 34px; }
...
<nav class="navbar">
    ...
    <span class="nav-title">Metti in vendita</span>
    <button class="btn-publish" id="btn-publish" form="shop-form" type="submit">Pubblica</button>
</nav>
...
<div class="form-wrap">
    ...
    <form id="shop-form" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" placeholder="Es. Set di coltelli da chef"
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="desc-input"
                      placeholder="Descrivi l'articolo, le condizioni, le specifiche..."
...
descInput.addEventListener('input', () => {
    charNum.textContent = descInput.value.length;
});
charNum.textContent = descInput.value.length;
