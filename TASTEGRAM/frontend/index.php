...
<nav class="navbar">
    <a href="index.php" class="navbar-logo">tastegram</a>
...
<div class="filter-bar">
    ...
    <a href="index.php?filter=friends"
...
// }
function toggleLike(postId, btn) {
    // ABBIAMO AGGIUNTO ../ PERCHÉ SIAMO DENTRO FRONTEND/
    fetch('../backend/api/like.php', {
        method: 'POST',
        headers: { 
    ...
    .then(r => {
        if (!r.ok) {
            throw new Error('Risposta del server non valida. Stato: ' + r.status);
        }
        return r.json();
    })
    .then(data => {
        if (data.success) {
            ...
            // Aggiorna il contatore dei mi piace specifico di questo post
            const countEl = document.getElementById('likes-count-' + postId);
            if (countEl) {
                countEl.textContent = data.likes_count + ' mi piace';
            }
        } else {
            alert('Errore dal database: ' + data.error);
        }
    })
    .catch((error) => {
        console.error('Dettaglio Errore Fetch:', error);
        // alert('Errore di rete sul like. Se l\'errore persiste, prova a rifare il Login.');
    });
}
